...
def FourierI(Array):
    ...
    for k in range(fields):
        for j in range(len(Array)-1):
            IReturn[k] = IReturn[k]+(float(Array[j]))*math.sin((((2*math.pi*j)/CONSTANT)*k)/fields)
    return IReturn
...
for i in range(fields):
    ...
    # by 1/fields, or the ratio of the number of points I have
        newY[i] = newY[i] + (float(Y[k])*1/fields)*math.sin((((2*math.pi*i))*k)/fields)+(float(Y[k])*1/fields)*math.cos((((2*math.pi*i))*k)/fields) # imaginary part
#newY = (1/fields)*newY
...
#pylab.show()
#pylab.plot(f,Y)    # Fourier Transform
pylab.plot(X,newY)    # Inverse Fourier Transform
pylab.show()
