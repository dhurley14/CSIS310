...
for i in range(350): # next 28 
    ...
    #therefore I needed to multiply the points at that array index
    # by 1/fields, or the ratio of the number of points I have
        dasY[i] = dasY[i] + ((float(R_Points[k])/350)*math.cos(2*math.pi*k/fields*i)) + ((float(I_Points[k])/350)*math.sin(2*math.pi*k/fields*i))
...
for i in range(321,350):
    finalY[i] = (oneMoreY[i] + newPolyValY[i])/2
    """
if(finalY[322]>3.0):
    print finalY[322]
pylab.hold(True)

#pylab.plot(X,Y)             # Power Spectrum
